...
/*   Updated: 2024/11/20 22:28:23 by emichels         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int wall_bottom, t_ray *ray)
{
	...
	p_distance = ray->distance * fabs(cos(ray->angle - map->plr_angle));
	if (p_distance < 0.1f)
		p_distance = 0.1f;
	wall_height = (float)SCREEN_HEIGHT / p_distance;
	t.texture_y_start = ((wall_top - (SCREEN_HEIGHT / 2 - wall_height / 2))
